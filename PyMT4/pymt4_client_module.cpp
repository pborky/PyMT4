...
typedef std::list<std::pair<std::string, boost::python::object>> PyShutdownHandlerList;
...
bool RegisterOnTickHandler(const std::string& symbol, boost::python::object& handler)
{
    ...
    {
#ifdef _DEBUG
        std::cout << __FILE__ << "," << __FUNCTION__ << ",L:" << __LINE__ << " Error: !PyCallable_Check" << std::endl;
#endif
        return false;
    }
    ...
    pyOnTickHandlerList.push_back(std::make_pair(symbol, handler));
    return true;
}
...
#ifdef _DEBUG
    std::cout << __FILE__ << "," << __FUNCTION__ << ",L:" << __LINE__ << " Error: Unexpected return" << std::endl;
#endif
    return false;
}
...
bool Connect(const uint32_t& port)
{
...

    {
        ...
        isConnected = ioConnector->connect(address, port);
...
{
#ifdef _DEBUG
    std::cout << __FILE__ << "," << __FUNCTION__ << ",L:" << __LINE__ << " Info: OK -> Disconnect()" << std::endl;
#endif
...
BOOST_PYTHON_MODULE(PyMT4)
{
    ...
    def("Is_Server_Online", &Is_Server_Online);
    def("Is_Server_Asleep", &Is_Server_Asleep);
    ...
    def("ExpertRemove", &Command::ExpertRemove);
    def("TerminalClose", &Command::TerminalClose);
    ...
    enum_<Constant>("Constant")
        ...
        .value("VALUE_EMPTY_VALUE",VALUE_EMPTY_VALUE)
        .value("VALUE_CLR_NONE",VALUE_CLR_NONE)
        .value("VALUE_WHOLE_ARRAY",VALUE_WHOLE_ARRAY)
        .export_values();
